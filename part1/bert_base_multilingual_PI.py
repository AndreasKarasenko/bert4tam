...
BI = df.loc[:,["content","PI", "train"]]
BIY = df.loc[:,["content","PIY", "train"]]
BIY.columns = ["content","PI", "train"]
BIZ = df.loc[:,["content","PIZ", "train"]]
BIZ.columns = ["content","PI", "train"]

data = pd.concat([BI,BIY,BIZ]).reset_index(drop=True)
print(data.shape)
data = data.loc[:,["content","PI","train"]]

# data = df.loc[:,["content","PI","train"]]
...
if "bert_base_multilingual_pi.index" in os.listdir("./final_eval/pretrained/"):
    model.load_weights("./final_eval/pretrained/bert_base_multilingual_pi")
...
if "bert_base_multilingual_pi.index" not in os.listdir("./final_eval/pretrained/"):
    model.fit(train, epochs=3)
    model.save_weights("./final_eval/pretrained/bert_base_multilingual_pi")
