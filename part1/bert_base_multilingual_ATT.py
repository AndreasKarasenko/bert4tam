...
BI = df.loc[:,["content","ATT", "train"]]
BIY = df.loc[:,["content","ATTY", "train"]]
BIY.columns = ["content","ATT", "train"]
BIZ = df.loc[:,["content","ATTZ", "train"]]
BIZ.columns = ["content","ATT", "train"]

data = pd.concat([BI,BIY,BIZ]).reset_index(drop=True)
print(data.shape)
# data = pd.concat([ATT,ATTY,ATTZ]).reset_index(drop=True)
data = data.loc[:,["content","ATT","train"]]

# data = df.loc[:,["content","ATT","train"]] # use for single non repeated constr
...
if "bert_base_multilingual_att.index" in os.listdir("./final_eval/pretrained/"):
    model.load_weights("./final_eval/pretrained/bert_base_multilingual_att")
...
if "bert_base_multilingual_att.index" not in os.listdir("./final_eval/pretrained/"):
    model.fit(train, epochs=3)
    model.save_weights("./final_eval/pretrained/bert_base_multilingual_att")
