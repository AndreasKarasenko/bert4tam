...
construct_type = "score"

# duplicate 
df = pd.read_excel("./final_eval/20231113_IKEA_BASE.xlsx", index_col=0)
# uncomment the BI lines if you want non repeated
score = df.loc[:,["content","score", "train"]]
scoreY = df.loc[:,["content","scoreY", "train"]]
scoreY.columns = ["content","score", "train"]
scoreZ = df.loc[:,["content","scoreZ", "train"]]
scoreZ.columns = ["content","score", "train"]

data = pd.concat([score,scoreY,scoreZ]).reset_index(drop=True)
print(data.shape)
data = data.loc[:,["content","score","train"]]

# data = df.loc[:,["content","score","train"]] # use for single non repeated constr
