...
BI = df.loc[:,["content","PEOU", "train"]]
BIY = df.loc[:,["content","PEOUY", "train"]]
BIY.columns = ["content","PEOU", "train"]
BIZ = df.loc[:,["content","PEOUZ", "train"]]
BIZ.columns = ["content","PEOU", "train"]

data = pd.concat([BI,BIY,BIZ]).reset_index(drop=True)
print(data.shape)
data = data.loc[:,["content","PEOU","train"]]

# data = df.loc[:,["content","PEOU","train"]] # use for single non repeated constr
...
if "bert_base_multilingual_peou.index" in os.listdir("./final_eval/pretrained/"):
    model.load_weights("./final_eval/pretrained/bert_base_multilingual_peou")
...
if "bert_base_multilingual_peou.index" not in os.listdir("./final_eval/pretrained/"):
    model.fit(train, epochs=3)
    model.save_weights("./final_eval/pretrained/bert_base_multilingual_peou")
