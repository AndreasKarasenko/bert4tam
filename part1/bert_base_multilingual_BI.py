...
BI = df.loc[:,["content","BI", "train"]]
BIY = df.loc[:,["content","BIY", "train"]]
BIY.columns = ["content","BI", "train"]
BIZ = df.loc[:,["content","BIZ", "train"]]
BIZ.columns = ["content","BI", "train"]

data = pd.concat([BI,BIY,BIZ]).reset_index(drop=True)
print(data.shape)
data = data.loc[:,["content","BI","train"]]

# data = df.loc[:,["content","BI","train"]]
...
if "bert_base_multilingual_bi.index" in os.listdir("./final_eval/pretrained/"):
    model.load_weights("./final_eval/pretrained/bert_base_multilingual_bi")
...
if "bert_base_multilingual_bi.index" not in os.listdir("./final_eval/pretrained/"):
    model.fit(train, epochs=3)
    model.save_weights("./final_eval/pretrained/bert_base_multilingual_bi")
