...
PU = df.loc[:,["content","PU", "train"]]
PUY = df.loc[:,["content","PUY", "train"]]
PUY.columns = ["content","PU", "train"]
PUZ = df.loc[:,["content","PUZ", "train"]]
PUZ.columns = ["content","PU", "train"]

data = pd.concat([PU,PUY,PUZ]).reset_index(drop=True)
print(data.shape)
data = data.loc[:,["content","PU","train"]]

# data = df.loc[:,["content","PU","train"]] # use for single non repeated constr
...
if "bert_base_multilingual_pu.index" in os.listdir("./final_eval/pretrained/"):
    model.load_weights("./pretrained/bert_base_multilingual_pu")
...
if "bert_base_multilingual_pu.index" not in os.listdir("./final_eval/pretrained/"):
    model.fit(train, epochs=3)
    model.save_weights("./final_eval/pretrained/bert_base_multilingual_pu")
